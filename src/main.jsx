...
if(localUrl){
  try{
    setStatus("Connecting to your configured cloud GPU…");
    const form=new FormData(); form.append("image",blob,"mira-reference.jpg"); form.append("prompt",prompt); form.append("aspect",aspect); form.append("steps","12"); form.append("duration",String(Math.min(Number(duration)||3.5,4)));
    const response=await fetch(localUrl.replace(/\/$/,"")+"/generate",{method:"POST",body:form});
    if(!response.ok) throw new Error("Configured GPU server returned HTTP "+response.status);
    const data=await response.json();
    if(data.video_url){setVideo(new URL(data.video_url,localUrl).href);setJob({requestId:"cloud-gpu-"+Date.now()});setStatus("AI film ready.");return;}
    if(!data.job_id) throw new Error("Configured GPU server returned no job id.");
    let job=data.job_id;
    for(let attempt=0;attempt<180;attempt++){
      await new Promise(r=>setTimeout(r,2000));
      const sr=await fetch(localUrl.replace(/\/$/,"")+"/jobs/"+job);
      if(!sr.ok) throw new Error("Could not read cloud GPU job status.");
      const st=await sr.json(); if(st.message) setStatus(st.message);
      if(st.status==="completed"&&st.video_url){setVideo(new URL(st.video_url,localUrl).href);setJob({requestId:job});setStatus("AI film ready.");return;}
...
for(let pi=0;pi<providers.length;pi++){
 ...
 try{
  setStatus(pi===0?"Connecting to Wan 2.2 free GPU…":"Primary GPU unavailable — trying backup Wan 2.2 server…");
  const client=await Client.connect(provider.space);
  result=await client.predict("/generate_video",provider.args(blob,prompt,negative,duration));
  if(result) break;
 }catch(providerError){
  lastError=String(providerError?.message||providerError||"");
  const quota=/zerogpu|quota|runs limit|exceeded.*limit|authenticate.*token|401|unauthorized/i.test(lastError);
  if(pi===providers.length-1){
   if(quota) throw new Error("All available free public Wan GPU servers are currently unavailable or quota-limited. Mira tried multiple online Wan 2.2 backends automatically. No Mira token or payment is required. Please retry later.");
   throw providerError;
  }
 }
}
if(!result) throw new Error(lastError||"No GPU backend is configured. Mira needs a cloud GPU endpoint to generate Wan 2.2 online.");const values=result?.data||result;let raw=Array.isArray(values)?values[0]:values;let found=raw?.video?.url||raw?.video?.path||raw?.url||raw?.path||raw?.data?.url||raw?.data?.path||"";if(!found&&Array.isArray(values)){for(const v of values){if(typeof v==="string"&&/^https?:\/\//.test(v)){found=v;break}if(v?.url){found=v.url;break}}}if(!found)throw new Error("Free Wan 2.2 returned no video. The public GPU may be busy or its free quota may be exhausted.");setVideo(found);setJob({requestId:"free-wan-"+Date.now()});setStatus("AI film ready — free Wan 2.2.");}catch(e){const message=String(e?.message||e||"Free Wan generation failed");setError(message);setStatus(message.startsWith("Free GPU quota reached")?"Generation paused — public free GPU quota reached.":"");}finally{setBusy(false)}}; return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Free Wan 2.2 • Public GPU queue • Reference Vision</div></header>
 ...
 <div className="card stage"><div className="cardhead"><span>03</span><h2>AI generation</h2><div className="modeltag">Wan 2.2 • Free public GPU</div></div><div className="preview">{video?<>{playableVideo?<video src={playableVideo} poster={preview} controls autoPlay loop muted playsInline preload="auto"/>:<div className="videoLoading">Loading generated video…</div>}<a className="videoOpen" href={video} target="_blank" rel="noreferrer">Open generated video</a></>:preview?<div className="sourcepreview"><img src={preview}/><span>Validated reference • {dims.w}×{dims.h}</span></div>:<div className="empty"><Film size={35}/><p>Final film preview</p><small>Your generated MP4 appears here. If preview does not load, use Open generated video.</small></div>}</div>
 ...
 <p className="fineprint">Free generation uses a primary Wan 2.2 public server with an automatic backup server. If both public GPU quotas are exhausted, Mira stops cleanly instead of asking for a token or payment.</p>
