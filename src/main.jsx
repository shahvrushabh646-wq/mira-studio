...
function renderBrowserMotion({imageUrl,aspect,duration,motion,intensity,onProgress}){
 return new Promise((resolve,reject)=>{
  ...
  image.onload=()=>{
   const canvas=document.createElement("canvas");canvas.width=aspect==="9:16"?540:960;canvas.height=aspect==="9:16"?960:540;
   ...
   let recorder;try{recorder=new MediaRecorder(stream,{mimeType,videoBitsPerSecond:5000000})}catch(e){stream.getTracks().forEach(t=>t.stop());reject(e);return}
...
function App(){
 ...
 const dims=engine==="browser"?(aspect==="9:16"?{w:540,h:960}:{w:960,h:540}):(aspect==="9:16"?{w:1080,h:1920}:{w:1920,h:1080});
...
return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>{engine==="browser"?"Free local video mode":"Wan 2.2 • Dedicated GPU mode"}</div></header>
 ...
 <div className="promptLabel"><label>Creative direction <span>Tell Mira exactly how you want the image to become a video</span></label><textarea className="creativePrompt" value={brief} onChange={e=>setBrief(e.target.value)} placeholder="Example: Make the teacher start writing on the board, students look toward the board, camera slowly moves from the back of the classroom toward the teacher, natural hand and head movement, realistic lighting, no new people or objects."/></div>{settings&&<div className="advancedSettings"><div className="settings"><label>Visual style<select value={style} onChange={e=>setStyle(e.target.value)}><option>Cinematic</option><option>Natural / realistic</option><option>Documentary</option><option>Dramatic</option><option>Premium commercial</option><option>Editorial</option></select></label><label>Motion language<select value={motion} onChange={e=>setMotion(e.target.value)}><option>Cinematic</option><option>Subtle</option><option>Dynamic</option><option>Subject follow</option><option>Orbit / tracking</option><option>Macro / detail</option></select></label></div><div className="settings"><label className="rangeLabel">Motion intensity <b>{intensity}%</b><input type="range" min="0" max="100" value={intensity} onChange={e=>setIntensity(+e.target.value)}/></label><label>Aspect ratio<select value={aspect} onChange={e=>setAspect(e.target.value)}><option>9:16</option><option>16:9</option></select></label></div><div className="settings"><label>Film duration<select value={duration} onChange={e=>setDuration(+e.target.value)}><option value="4">4 sec • 1 segment</option><option value="7">7 sec • 2 segments</option><option value="15">15 sec • 4 segments</option><option disabled={engine==="browser"} value="30">30 sec • 8 segments</option><option disabled={engine==="browser"} value="60">60 sec • 15 segments</option><option disabled={engine==="browser"} value="120">120 sec • 20 segments</option><option disabled={engine==="browser"} value="300">300 sec • 49 segments</option></select></label><label>Output quality<select value={quality} onChange={e=>setQuality(e.target.value)}><option value="draft">Fast draft • 8 steps</option><option value="professional">Professional • 40 steps</option></select></label></div><div className="engineControls"><label>Generation mode<select value={engine} onChange={e=>{setEngine(e.target.value);if(e.target.value==="browser"&&duration>15)setDuration(15);setPlan(null);setVideo("");setError("")}}><option value="browser">Free motion film - no GPU or account</option><option value="personal">AI video - Wan 2.2 GPU server</option></select></label>{engine==="personal"?<><label>Wan 2.2 GPU API URL<input value={localEngineUrl} onChange={e=>{setLocalEngineUrl(e.target.value);setGpuConnection("")}} placeholder="https://your-dedicated-wan-gpu.example.com"/></label><div className="gpuConnect"><button className="secondary" disabled={checkingGpu||busy} onClick={testGpuConnection}>{checkingGpu?<><Loader2 className="spin" size={15}/> Checking GPU...</>:"Test GPU connection"}</button>{gpuConnection&&<p className={gpuConnection.startsWith("Ready")?"gpuReady":"gpuWarning"}>{gpuConnection}</p>}</div><label>GPU API key (if required)<input type="password" value={gpuApiKey} onChange={e=>{setGpuApiKey(e.target.value);setGpuConnection("")}} placeholder="Only for your dedicated GPU"/></label><label>AI engine status<input value={"Wan 2.2 - real generated subject motion"} readOnly/></label></>:<div className="browserModeNote"><b>Runs on this page</b><span>Your image stays in your browser. Mira exports a smooth camera-motion video as WebM. People and objects remain still; AI-generated actions require a GPU video model.</span></div>}<label className="negativeLabel">Negative prompt<input value={negative} onChange={e=>setNegative(e.target.value)}/></label></div></div>}
 <div className="framecheck"><Check size={15}/><span>Delivery frame: <b>{dims.w} × {dims.h}</b> • {aspect}</span></div>
 <p className="fineprint visionDisclosure">{engine==="browser"?"No upload, API key or GPU: Mira makes a camera-motion plan locally from your direction. This creates motion graphics, not new AI-generated subject actions.":"Image analysis uses the public Hugging Face Qwen Vision Space. Video generation stays on your configured Wan 2.2 GPU."}</p><button className="primary" disabled={!file||busy} onClick={buildPlan}><Sparkles size={18}/> {busy?"Studying reference…":plan?"Refresh director plan":engine==="browser"?"Create free local plan":"Study image & build director plan"}</button>
 </div>
 ...
 <div className="card stage"><div className="cardhead"><span>03</span><h2>{engine==="browser"?"Local video render":"AI generation"}</h2><div className="modeltag">{engine==="browser"?"Browser - no GPU":engine==="personal"?"Wan 2.2 - Personal GPU":"Wan 2.2 - Real AI I2V"}</div></div><div className="preview">{video?<>{playableVideo?<video src={playableVideo} poster={preview} controls autoPlay loop muted playsInline preload="auto"/>:<div className="videoLoading">Loading generated video…</div>}<a className="videoOpen" href={video} target="_blank" rel="noreferrer">Open generated video</a></>:preview?<div className="sourcepreview"><img src={preview}/><span>Validated reference • {dims.w}×{dims.h}</span></div>:<div className="empty"><Film size={35}/><p>Final film preview</p><small>Your generated film appears here. If the embedded preview does not load, use Open generated video.</small></div>}</div>
<div className="actions"><button className="primary" disabled={!file||!plan||busy} onClick={generate}>{busy?<><Loader2 className="spin" size={18}/> {status||"Generating…"}</>:<><Play size={18}/> {engine==="browser"?"Render motion film":"Generate AI film"}</>}</button>{video&&<a className="secondary" href={video} target="_blank" rel="noreferrer" download={engine==="browser"?"mira-motion-film.webm":"mira-ai-film.mp4"}><Download size={16}/> {engine==="browser"?"Save WebM":"Save MP4"}</a>}</div>
 <div className="status">{status&&<><span className="statusdot"/>{status}</>}{job&&<span className="request">Request {job.requestId.slice(0,8)}…</span>}</div><div className="generationNotes">{engine==="browser"?<><span><Check size={13}/> Local browser rendering</span><span><Check size={13}/> Cinematic camera movement</span><span><Check size={13}/> WebM export</span></>:<>{engine==="browser"?<><span><Check size={13}/> Local browser rendering</span><span><Check size={13}/> Cinematic camera movement</span><span><Check size={13}/> WebM export</span></>:<><span><Check size={13}/> Real AI image-to-video</span><span><Check size={13}/> Scene-specific shot direction</span><span><Check size={13}/> Identity / text protection</span></>}</>}</div>{error&&<div className="error"><AlertTriangle size={16}/>{error}</div>}
 <p className="fineprint">{engine==="browser"?"Free local mode creates a polished camera-motion video from your still image. It does not invent or animate people and objects.":"AI video mode sends generation requests to your configured Wan 2.2 GPU server."}</p>
