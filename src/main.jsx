...
function App(){
 const[file,setFile]=useState(null),[preview,setPreview]=useState(""),[playableVideo,setPlayableVideo]=useState(""),[engine,setEngine]=useState("free"),[hfToken,setHfToken]=useState(""),[localEngineUrl,setLocalEngineUrl]=useState("http://127.0.0.1:8000"),[gpuApiKey,setGpuApiKey]=useState(""),[brief,setBrief]=useState("Create a premium cinematic scene based strictly on the uploaded image. Understand what is actually happening first, then animate only actions that logically belong to the visible people, objects, environment and setting. Preserve identities, proportions, composition and visible wording. Use realistic lighting, depth, physically plausible motion and clean editorial cuts. Do not invent unrelated content.");
...
for(let pi=0;pi<shuffledProviders.length;pi++){
  ...
  try{
   ...
   if(/ZeroGPU quota|quota exceeded|requested vs\.|remaining quota/i.test(lastError)){throw new Error("The free AI GPU quota is currently unavailable. Mira will not replace real AI generation with a zoom/pan animation. Try again later or connect a Wan 2.2 GPU endpoint.");}}const draw=()=>{if(v.ended)return;const ctx=canvas.getContext("2d");ctx.drawImage(v,0,0,canvas.width,canvas.height);requestAnimationFrame(draw)};draw();});}
  recorder.stop();await done;const blob=new Blob(chunks,{type:"video/webm"});return URL.createObjectURL(blob);
};
const createFreeMotionFallback=async(imageUrl,totalDuration)=>{const response=await fetch(imageUrl);if(!response.ok)throw new Error("Could not load the reference image for free fallback mode.");const imageBlob=await response.blob();const img=new Image();img.src=URL.createObjectURL(imageBlob);await new Promise((res,rej)=>{img.onload=res;img.onerror=rej});const w=aspect==="9:16"?720:960,h=aspect==="9:16"?1280:540;const canvas=document.createElement("canvas");canvas.width=w;canvas.height=h;const ctx=canvas.getContext("2d");const stream=canvas.captureStream(30);const chunks=[];const mime=MediaRecorder.isTypeSupported("video/webm;codecs=vp9")?"video/webm;codecs=vp9":"video/webm";const recorder=new MediaRecorder(stream,{mimeType:mime});recorder.ondataavailable=e=>e.data.size&&chunks.push(e.data);const done=new Promise(r=>recorder.onstop=r);recorder.start();const start=performance.now();const ms=Math.max(3500,Number(totalDuration)*1000);const draw=now=>{const p=Math.min(1,(now-start)/ms);const phase=p*Math.PI*2;const zoom=1.02+0.045*(0.5-0.5*Math.cos(phase));const panX=Math.sin(phase*0.65)*w*0.018;const panY=Math.cos(phase*0.55)*h*0.012;const scale=Math.max(w/img.width,h/img.height)*zoom;const dw=img.width*scale,dh=img.height*scale;ctx.fillStyle="#09090b";ctx.fillRect(0,0,w,h);ctx.save();ctx.translate(w/2+panX,h/2+panY);ctx.rotate(Math.sin(phase*0.5)*0.0025);ctx.drawImage(img,-dw/2,-dh/2,dw,dh);ctx.restore();if(p<1)requestAnimationFrame(draw);else recorder.stop()};requestAnimationFrame(draw);await done;URL.revokeObjectURL(img.src);return URL.createObjectURL(new Blob(chunks,{type:mime}))};
const generate=async()=>{if(!file||!plan)return;setBusy(true);setError("");setVideo("");try{setStatus("Preparing reference image…");const imageData=await makePreparedData();const blob=await(await fetch(imageData)).blob();const shotDirection=(plan?.shots||[]).map((s,i)=>`SHOT ${i+1} — ${s.name} (${s.duration}s): ACTION: ${s.action} CAMERA: ${s.camera}`).join("\n");
...
setVideo(finalVideo);setJob({requestId:"mira-long-film-"+Date.now(),segments:segmentUrls.length});setStatus("AI film ready — "+requestedDuration+"s requested.");}catch(e){const message=String(e?.message||e||"Free Wan generation failed");setError(message);setStatus("");}finally{setBusy(false)}};return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Free Wan 2.2 • Live public GPU discovery • Reference Vision</div></header>
 ...
 <div className="promptLabel"><label>Creative direction <span>Tell Mira exactly how you want the image to become a video</span></label><textarea className="creativePrompt" value={brief} onChange={e=>setBrief(e.target.value)} placeholder="Example: Make the teacher start writing on the board, students look toward the board, camera slowly moves from the back of the classroom toward the teacher, natural hand and head movement, realistic lighting, no new people or objects."/></div>{settings&&<div className="advancedSettings"><div className="settings"><label>Visual style<select value={style} onChange={e=>setStyle(e.target.value)}><option>Cinematic</option><option>Natural / realistic</option><option>Documentary</option><option>Dramatic</option><option>Premium commercial</option><option>Editorial</option></select></label><label>Motion language<select value={motion} onChange={e=>setMotion(e.target.value)}><option>Cinematic</option><option>Subtle</option><option>Dynamic</option><option>Subject follow</option><option>Orbit / tracking</option><option>Macro / detail</option></select></label></div><div className="settings"><label className="rangeLabel">Motion intensity <b>{intensity}%</b><input type="range" min="0" max="100" value={intensity} onChange={e=>setIntensity(+e.target.value)}/></label><label>Aspect ratio<select value={aspect} onChange={e=>setAspect(e.target.value)}><option>9:16</option><option>16:9</option></select></label></div><div className="settings"><label>Film duration<select value={duration} onChange={e=>setDuration(+e.target.value)}><option value="3.5">3.5 sec</option><option value="7">7 sec • 2 segments</option><option value="15">15 sec • 5 segments</option><option value="30">30 sec • 10 segments</option><option value="60">60 sec • 20 segments</option></select></label><label>Output quality<select defaultValue="standard"><option>Standard • fast</option><option>High • slower</option></select></label></div><div className="engineControls"><label>Generation engine<select value={engine} onChange={e=>setEngine(e.target.value)}><option value="free">Wan 2.2 • Public free AI GPU</option><option value="personal">Wan 2.2 • Personal GPU</option></select></label><label>Personal GPU API URL<input value={localEngineUrl} onChange={e=>setLocalEngineUrl(e.target.value)} placeholder="http://127.0.0.1:8000"/></label><label>Personal GPU API key (optional)<input type="password" value={gpuApiKey} onChange={e=>setGpuApiKey(e.target.value)} placeholder="MIRA_GPU_API_KEY"/></label><label>Generation mode<input value={engine==="personal"?"Personal GPU • private queue + VRAM-aware Wan":"Free public GPU • anonymous/shared quota"} readOnly/></label><label>HF token (optional)<input type="password" value={hfToken} onChange={e=>setHfToken(e.target.value)} placeholder="Paste hf_… to use your own free quota"/><small>Kept only in this browser session. A token does not create unlimited GPU; free HF accounts currently have a daily ZeroGPU quota.</small></label></div><label className="negativeLabel">Negative prompt<input value={negative} onChange={e=>setNegative(e.target.value)}/></label></div>}
