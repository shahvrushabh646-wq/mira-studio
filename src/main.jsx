...
const buildPlan=async()=>{
 ...
 const generate=async()=>{if(!file||!plan)return;setBusy(true);setError("");setVideo("");try{setStatus("Preparing reference image…");const imageData=await makePreparedData();const blob=await(await fetch(imageData)).blob();const prompt=("Study this exact reference image first. "+(visualAnalysis||plan.reference||"Preserve the visible scene faithfully.")+". "+brief+". Animate only actions that logically belong to the visible people, objects and environment. Preserve identities, proportions, composition and visible text. "+selectedMotionText(style,motion,intensity)+" "+negative).slice(0,5000);setStatus("Connecting to free public Wan 2.2…");const spaces=["dream2589632147/Dream-wan2-2-fp8da-aoti-preview-2","zerogpu-aoti/wan2-2-fp8da-aoti-faster"];let last="";for(const space of spaces){try{setStatus("Free Wan 2.2 — trying "+space.split("/")[0]+"…");const app=await Client.connect(space);const payload={input_image:handle_file(blob),prompt,steps:6,negative_prompt:negative,duration_seconds:Math.min(Number(duration)||4,4.5),guidance_scale:1,guidance_scale_2:1,seed:Math.floor(Math.random()*2147483647),randomize_seed:true};const prediction=await app.predict("/generate_video",payload);const result=prediction?.data??prediction;const out=Array.isArray(result)?result[0]:result;const url=out?.url||out?.path||out?.video?.url||out?.video?.path||out?.file?.url||out?.file?.path||out?.data?.url||out?.data?.path;if(url){setVideo(url);setJob({requestId:"free-wan-"+Date.now()});setStatus("AI film ready — free Wan 2.2 public GPU.");return}}catch(e){last=e?.message||String(e)}}throw new Error("Free Wan 2.2 services are currently unavailable or queued. "+last)}catch(e){setError(e?.message||"Something went wrong");setStatus("")}finally{setBusy(false)}}; return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Wan 2.2 • Unlimited backend • No user token • Reference Vision</div></header>
