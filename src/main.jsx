...
function App(){
 const[file,setFile]=useState(null),[preview,setPreview]=useState(""),[playableVideo,setPlayableVideo]=useState(""),[brief,setBrief]=useState("Create a premium cinematic scene based strictly on the uploaded image. Understand what is actually happening first, then animate only actions that logically belong to the visible people, objects, environment and setting. Preserve identities, proportions, composition and visible wording. Use realistic lighting, depth, physically plausible motion and clean editorial cuts. Do not invent unrelated content.");
...
const buildPlan=async()=>{
 ...
 const generate=async()=>{if(!file)return;setBusy(true);setError("");setVideo("");try{setStatus("Preparing validated reference frame…");const imageData=await makePreparedData();setStatus(`Reference frame prepared: ${dims.w} × ${dims.h} — ${aspect}.`);const blob=await (await fetch(imageData)).blob();setStatus("AI is using the director-approved reference sheet and generating the scene…");const imageAnalysis=visualAnalysis||plan?.reference||"Reference image is the source of truth. Preserve all visible people, objects, environment and clearly visible wording exactly.";const app=await Client.connect("zerogpu-aoti/wan2-2-fp8da-aoti-faster");const submission=app.submit("/generate_video",{input_image:handle_file(blob),prompt:("FIRST STUDY AND FOLLOW THIS REFERENCE IMAGE FACTUALLY. REFERENCE SHEET: "+imageAnalysis+". USER BRIEF: "+brief+". SCENE DIRECTOR: Create a natural cinematic short from this exact image, not necessarily an advertisement. First determine what is actually happening in the image, then animate that scene logically. Use six distinct visual beats inside this 5-second film: establish the scene, animate the main subject, show a meaningful interaction/action, cut to a useful detail, build a natural progression, then resolve on the scene. Use hard editorial cuts only when they help; never morph objects or people. Preserve identities, faces, clothing, body proportions, object geometry, environment, visible wording and spatial relationships. Do not invent unrelated people, objects, logos, locations or text. If it is a classroom/lecture, animate teaching, writing, reading, students reacting and classroom ambience. If it is a person/portrait, use natural human movement and expression. If it is an event, animate the event activity and crowd. If it is nature, animate physically plausible environmental and subject movement. If it is an object/product, animate its real-world action rather than forcing an ad story. The action must come from the image itself. "+selectedMotionText(style,motion,intensity)+" Negative constraints: "+negative+". Animate only what is physically compatible with the reference image.").slice(0,7000),steps:6,negative_prompt:negative,duration_seconds:5,guidance_scale:1,guidance_scale_2:1,seed:Math.floor(Math.random()*2147483647),randomize_seed:true});let result=null;for await(const event of submission){if(event.type==="status"){const st=event.stage||event.status?.stage||event.status;setStatus(st==="generating"?"Free GPU is rendering the video…":"Free GPU queue: "+String(st||"working")+"…")}if(event.type==="data"&&event.data){result=event.data;setStatus("Free GPU returned a result — preparing video…")}}if(!result)throw new Error("Free GPU finished without returning a video file. The Hugging Face Space may be temporarily busy or have changed its output format. Please try again.");const out=result[0]||result;const url=out?.url||out?.path||out?.video?.url||out?.video?.path;if(!url)throw new Error("Free GPU completed but returned no video URL.");setVideo(url);setStatus("AI film ready — generated on Hugging Face ZeroGPU.");setJob({requestId:"free-hf"});}catch(e){setError(e.message||"Something went wrong");setStatus("")}finally{setBusy(false)}};
 return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Wan 2.2 Fast • Free GPU • Reference Vision</div></header>
 ...
 <textarea value={brief} onChange={e=>setBrief(e.target.value)}/>{settings&&<div className="advancedSettings"><div className="settings"><label>Visual style<select value={style} onChange={e=>setStyle(e.target.value)}><option>Cinematic</option><option>Natural / realistic</option><option>Documentary</option><option>Dramatic</option><option>Premium commercial</option><option>Editorial</option></select></label><label>Motion language<select value={motion} onChange={e=>setMotion(e.target.value)}><option>Cinematic</option><option>Subtle</option><option>Dynamic</option><option>Subject follow</option><option>Orbit / tracking</option><option>Macro / detail</option></select></label></div><div className="settings"><label className="rangeLabel">Motion intensity <b>{intensity}%</b><input type="range" min="0" max="100" value={intensity} onChange={e=>setIntensity(+e.target.value)}/></label><label>Aspect ratio<select value={aspect} onChange={e=>setAspect(e.target.value)}><option>9:16</option><option>16:9</option></select></label></div><div className="settings"><label>Film duration<select value={duration} onChange={e=>setDuration(+e.target.value)}><option value="5">5 sec • Free Fast Mode</option></select></label><label>Output quality<select defaultValue="standard"><option>Standard • fast</option><option>High • slower</option></select></label></div><label className="negativeLabel">Negative prompt<input value={negative} onChange={e=>setNegative(e.target.value)}/></label></div>}
