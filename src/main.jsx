...
const buildPlan=async()=>{
 ...
 const generate=async()=>{if(!file||!plan)return;setBusy(true);setError("");setVideo("");try{setStatus("Preparing reference image for the cloud Wan 2.2 engine…");const imageData=await makePreparedData();setStatus("Sending to Mira's cloud Wan 2.2 engine…");const r=await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({imageData,brief,shots:plan.shots,duration,style,motion,intensity,negativePrompt:negative,aspect})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||"Cloud video backend failed.");setJob({requestId:data.requestId});let done=false;while(!done){await new Promise(x=>setTimeout(x,2500));const s=await fetch("/api/status?requestId="+encodeURIComponent(data.requestId));const j=await s.json().catch(()=>({}));if(!s.ok)throw new Error(j.error||"Could not read video job status.");if(j.status==="COMPLETED"){if(!j.videoUrl)throw new Error("Wan 2.2 finished but returned no video URL.");setVideo(j.videoUrl);done=true;setStatus("AI film ready — generated by Mira's cloud Wan 2.2 engine.");}else if(j.status==="FAILED"||j.status==="ERROR"){throw new Error(j.error||"Wan 2.2 generation failed.");}else{setStatus("Wan 2.2 cloud generation — "+String(j.status||"processing").toLowerCase()+"…");}}}catch(e){setError(e?.message||"Something went wrong");setStatus("");}finally{setBusy(false)}}; return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Wan 2.2 • Unlimited backend • No user token • Reference Vision</div></header>
 ...
 <textarea value={brief} onChange={e=>setBrief(e.target.value)}/>{settings&&<div className="advancedSettings"><div className="settings"><label>Visual style<select value={style} onChange={e=>setStyle(e.target.value)}><option>Cinematic</option><option>Natural / realistic</option><option>Documentary</option><option>Dramatic</option><option>Premium commercial</option><option>Editorial</option></select></label><label>Motion language<select value={motion} onChange={e=>setMotion(e.target.value)}><option>Cinematic</option><option>Subtle</option><option>Dynamic</option><option>Subject follow</option><option>Orbit / tracking</option><option>Macro / detail</option></select></label></div><div className="settings"><label className="rangeLabel">Motion intensity <b>{intensity}%</b><input type="range" min="0" max="100" value={intensity} onChange={e=>setIntensity(+e.target.value)}/></label><label>Aspect ratio<select value={aspect} onChange={e=>setAspect(e.target.value)}><option>9:16</option><option>16:9</option></select></label></div><div className="settings"><label>Film duration<select value={duration} onChange={e=>setDuration(+e.target.value)}><option value="5">5 sec • Short film</option></select></label><label>Output quality<select defaultValue="standard"><option>Standard • fast</option><option>High • slower</option></select></label></div><div className="engineControls"><label>Generation engine<select value={engine} onChange={e=>setEngine(e.target.value)}><option value="cloud">Mira Cloud • Wan 2.2</option></select></label><label>Generation mode<input value="Cloud backend • no user token" readOnly/></label></div><label className="negativeLabel">Negative prompt<input value={negative} onChange={e=>setNegative(e.target.value)}/></label></div>}
