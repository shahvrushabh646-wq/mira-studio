...
for(let pi=0;pi<providers.length;pi++){
 ...
 try{
  ...
  const quota=/zerogpu|quota|runs limit|exceeded.*limit|authenticate.*token/i.test(lastError);
  if(pi===providers.length-1){
   if(quota) throw new Error("Free GPU quota is currently exhausted on the available public Wan servers. Mira tried the primary and backup servers automatically. No Mira token or payment is required. Please retry after the public GPU quota resets.");
   throw providerError;
  }
 }
}
