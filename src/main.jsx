...
function App(){
 const[file,setFile]=useState(null),[preview,setPreview]=useState(""),[playableVideo,setPlayableVideo]=useState(""),[engine,setEngine]=useState("free"),[localEngineUrl,setLocalEngineUrl]=useState("http://127.0.0.1:8000"),[gpuApiKey,setGpuApiKey]=useState(""),[brief,setBrief]=useState("Create a premium cinematic scene based strictly on the uploaded image. Understand what is actually happening first, then animate only actions that logically belong to the visible people, objects, environment and setting. Preserve identities, proportions, composition and visible wording. Use realistic lighting, depth, physically plausible motion and clean editorial cuts. Do not invent unrelated content.");
...
return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Wan 2.2 • Real AI Film Director • Local/Free GPU</div></header>
 ...
 <div className="promptLabel"><label>Creative direction <span>Tell Mira exactly how you want the image to become a video</span></label><textarea className="creativePrompt" value={brief} onChange={e=>setBrief(e.target.value)} placeholder="Example: Make the teacher start writing on the board, students look toward the board, camera slowly moves from the back of the classroom toward the teacher, natural hand and head movement, realistic lighting, no new people or objects."/></div>{settings&&<div className="advancedSettings"><div className="settings"><label>Visual style<select value={style} onChange={e=>setStyle(e.target.value)}><option>Cinematic</option><option>Natural / realistic</option><option>Documentary</option><option>Dramatic</option><option>Premium commercial</option><option>Editorial</option></select></label><label>Motion language<select value={motion} onChange={e=>setMotion(e.target.value)}><option>Cinematic</option><option>Subtle</option><option>Dynamic</option><option>Subject follow</option><option>Orbit / tracking</option><option>Macro / detail</option></select></label></div><div className="settings"><label className="rangeLabel">Motion intensity <b>{intensity}%</b><input type="range" min="0" max="100" value={intensity} onChange={e=>setIntensity(+e.target.value)}/></label><label>Aspect ratio<select value={aspect} onChange={e=>setAspect(e.target.value)}><option>9:16</option><option>16:9</option></select></label></div><div className="settings"><label>Film duration<select value={duration} onChange={e=>setDuration(+e.target.value)}><option value="3.5">3.5 sec</option><option value="7">7 sec • 2 segments</option><option value="15">15 sec • 5 segments</option><option value="30">30 sec • 10 segments</option><option value="60">60 sec • 20 segments</option><option value="120">120 sec • long film</option><option value="300">300 sec • long film</option></select></label><label>Output quality<select defaultValue="standard"><option>Standard • fast</option><option>High • slower</option></select></label></div><div className="engineControls"><label>Generation engine<select value={engine} onChange={e=>setEngine(e.target.value)}><option value="free">Wan 2.2 • Public free AI GPU</option><option value="personal">Wan 2.2 • Personal GPU</option></select></label><label>Personal GPU API URL<input value={localEngineUrl} onChange={e=>setLocalEngineUrl(e.target.value)} placeholder="http://127.0.0.1:8000"/></label>{engine==="personal"&&<label>Dedicated GPU API key (optional)<input type="password" value={gpuApiKey} onChange={e=>setGpuApiKey(e.target.value)} placeholder="Only for your dedicated GPU"/></label>}<label>Generation mode<input value={engine==="personal"?"Personal GPU • private queue + VRAM-aware Wan":"Free public GPU • anonymous/shared quota"} readOnly/></label></div><label className="negativeLabel">Negative prompt<input value={negative} onChange={e=>setNegative(e.target.value)}/></label></div>}
 ...
 <p className="fineprint">GPU generation searches live public Wan Spaces and automatically skips incompatible, paused, or unavailable backends. Public GPU mode is best-effort and may have queues or quotas. Free mode uses no app credits and no token. Mira never substitutes a fake zoom/pan animation when a real AI GPU is unavailable. Unlimited generation requires the Personal GPU mode, where Mira sends every shot to your own compatible Wan 2.2 GPU server without an app-side generation limit.</p>
