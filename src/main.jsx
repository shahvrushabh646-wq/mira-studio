...
const prompt=("FINAL DIRECTOR INSTRUCTION. The uploaded image is the absolute visual source of truth. Do not treat it as a generic reference. Reconstruct and animate the exact scene described by the visual director analysis. VISUAL DIRECTOR SHEET:\n"+(visualAnalysis||plan.reference||"Preserve the visible scene faithfully.")+"\nUSER CREATIVE PROMPT / BRIEF:\n"+brief+"\nDIRECTOR SHOT PLAN:\n"+shotDirection+"\nFor every beat, follow the described subject action, camera angle, camera height, framing, lens perspective, lighting, environment motion and timing. The user brief controls the creative intent, while the image and director sheet control what is physically present. If the user asks for an action that conflicts with the visible image, adapt it to the closest physically plausible version using the visible subjects and environment. Preserve identity, anatomy, object geometry, composition, readable text and spatial relationships. Maintain continuity between beats. Use natural acceleration/deceleration, realistic depth and physically plausible interaction. Never add unrelated people/objects, invent text, morph subjects, or turn the scene into a product advertisement unless the image and user brief explicitly support that. "+selectedMotionText(style,motion,intensity)+" NEGATIVE CONSTRAINTS: "+negative).slice(0,10000);setStatus("Connecting to free Wan 2.2 GPU…");const client=await Client.connect("zerogpu-aoti/wan2-2-fp8da-aoti-faster");let result;try{result=await client.predict("/generate_video",[handle_file(blob),null,prompt,6,negative,Math.min(Number(duration)||3.5,3.5),1,1,Math.floor(Math.random()*2147483647),true,6,"UniPCMultistep",3,16,"4x-UltraSharp",1,true,false,true]);}catch(gpuError){const message=String(gpuError?.message||gpuError||"");const quota=/zerogpu|quota|runs limit|exceeded.*limit|authenticate.*token/i.test(message);if(quota){throw new Error("Free GPU quota reached. Hugging Face has temporarily limited this public Wan 2.2 run. No Mira token or payment is required. Please try again after the public quota resets.");}throw gpuError;}const values=result?.data||result;let raw=Array.isArray(values)?values[0]:values;let found=raw?.video?.url||raw?.video?.path||raw?.url||raw?.path||raw?.data?.url||raw?.data?.path||"";if(!found&&Array.isArray(values)){for(const v of values){if(typeof v==="string"&&/^https?:\/\//.test(v)){found=v;break}if(v?.url){found=v.url;break}}}if(!found)throw new Error("Free Wan 2.2 returned no video. The public GPU may be busy or its free quota may be exhausted.");setVideo(found);setJob({requestId:"free-wan-"+Date.now()});setStatus("AI film ready — free Wan 2.2.");}catch(e){const message=String(e?.message||e||"Free Wan generation failed");setError(message);setStatus(message.startsWith("Free GPU quota reached")?"Generation paused — public free GPU quota reached.":"");}finally{setBusy(false)}}; return <div className="app"><header><div className="brand"><div className="logo">M</div><div><b>Mira Studio</b><span>AI Image-to-Scene Director</span></div></div><div className="provider"><span className="dot"/>Free Wan 2.2 • Public GPU queue • Reference Vision</div></header>
 ...
 <p className="fineprint">Free cloud generation uses a shared Wan 2.2 GPU queue. If the public ZeroGPU quota is reached, Mira pauses generation and does not ask for a token or payment. Try again after the quota resets.</p>
