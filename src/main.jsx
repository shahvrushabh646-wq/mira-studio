...
const buildPlan=async()=>{
 ...
 const generate=async()=>{if(!file)return;setBusy(true);setError("");setVideo("");try{setStatus("Preparing validated reference frame…");const imageData=await makePreparedData();setStatus(`Reference frame prepared: ${dims.w} × ${dims.h} — ${aspect}.`);const blob=await (await fetch(imageData)).blob();setStatus("AI is using the director-approved reference sheet and generating the scene…");const imageAnalysis=visualAnalysis||plan?.reference||"Reference image is the source of truth. Preserve all visible people, objects, environment and clearly visible wording exactly.";const app=await Client.connect("zerogpu-aoti/wan2-2-fp8da-aoti-faster");const payload={input_image:handle_file(blob),prompt:("FIRST STUDY AND FOLLOW THIS REFERENCE IMAGE FACTUALLY. REFERENCE SHEET: "+imageAnalysis+". USER BRIEF: "+brief+". SCENE DIRECTOR: Create a natural cinematic short from this exact image, not necessarily an advertisement. First determine what is actually happening in the image, then animate that scene logically. Use six distinct visual beats inside this 5-second film: establish the scene, animate the main subject, show a meaningful interaction/action, cut to a useful detail, build a natural progression, then resolve on the scene. Use hard editorial cuts only when they help; never morph objects or people. Preserve identities, faces, clothing, body proportions, object geometry, environment, visible wording and spatial relationships. Do not invent unrelated people, objects, logos, locations or text. If it is a classroom/lecture, animate teaching, writing, reading, students reacting and classroom ambience. If it is a person/portrait, use natural human movement and expression. If it is an event, animate the event activity and crowd. If it is nature, animate physically plausible environmental and subject movement. If it is an object/product, animate its real-world action rather than forcing an ad story. The action must come from the image itself. "+selectedMotionText(style,motion,intensity)+" Negative constraints: "+negative+". Animate only what is physically compatible with the reference image.").slice(0,7000),steps:6,negative_prompt:negative,duration_seconds:5,guidance_scale:1,guidance_scale_2:1,seed:Math.floor(Math.random()*2147483647),randomize_seed:true};const prediction=await app.predict("/generate_video",payload);setStatus("Free GPU returned a result — preparing video…");const result=prediction?.data??prediction;const out=Array.isArray(result)?result[0]:result;const url=out?.url||out?.path||out?.video?.url||out?.video?.path||out?.file?.url||out?.file?.path||out?.data?.url||out?.data?.path;if(!url)throw new Error("Free GPU completed, but Mira could not read the returned MP4 path. The public Space returns a video path plus seed; the app now uses the direct prediction response.");setVideo(url);setStatus("AI film ready — generated on Hugging Face ZeroGPU.");setJob({requestId:"free-hf"});}catch(e){setError(e.message||"Something went wrong");setStatus("")}finally{setBusy(false)}};
