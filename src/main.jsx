...
function makeDirectorShots(analysis,brief,duration){
 ...
 const count=6;
 return template.slice(0,count).map((s,i)=>({name:s[0],action:s[1],camera:s[2],duration:Math.max(1,Math.floor(duration/count)+(i<duration%count?1:0))}));
}
...
const buildPlan=async()=>{
 ...
 <button className="drop" onClick={()=>fileRef.current.click()}>{preview?<img src={preview}/>:<><Upload size={28}/><b>Upload product photo</b><small>Class, lecture, people, event, product, nature — JPG, PNG or WebP</small></>}</button><input ref={fileRef} hidden type="file" accept="image/*" onChange={onFile}/>
