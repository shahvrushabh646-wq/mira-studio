...
function App(){
 ...
 const vision=await Client.connect("developer0hye/Qwen2.5-VL-7B-Instruct",hfToken.trim()?{token:hfToken.trim()}:undefined);
...
const buildPlan=async()=>{
 ...
 try{
  ...
  catch(e){const detail=String(e?.message||e||"Unknown Hugging Face error");const guidance=/quota|limit|zerogpu|gpu|429|503|busy|queue/i.test(detail)?"The Qwen Vision Space may be busy or out of free quota. Wait and retry, or add your Hugging Face read token in Generation mode.":/401|403|unauthor|token|credential/i.test(detail)?"Check that the optional Hugging Face token is valid and has read access.":"Check your internet connection and whether the Qwen Vision Space is available.";throw new Error(`Image analysis failed: ${detail}. ${guidance} Mira did not create a shot plan without analyzing the image.`)}
...
const generatePublicAI=async()=>{
 ...
 try{
  ...
  const client=await Client.connect("r3gm/wan2-2-fp8da-aoti-preview",hfToken.trim()?{token:hfToken.trim()}:undefined);
