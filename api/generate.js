...
export default async function handler(req,res){
 ...
 try{
  const {imageData,brief,shots,duration}=req.body||{};
  ...
  const input={
    ...
    multi_prompt:shots.map(s=>({prompt:`Product commercial shot. ${s.action} Camera: ${s.camera}. ${brief||""} Preserve exact product identity, packaging, proportions, materials and visible text. Photorealistic, premium advertising cinematography, physically plausible motion, stable geometry, no invented logos, no watermark.`,duration:Number(s.duration||1)})),
    duration:Math.min(15,Math.max(3,total)),
    shot_type:"customize",
    generate_audio:false,
    negative_prompt:"warped product, deformed packaging, duplicate product, invented logo, fake text, watermark, distorted hands, melting, morphing, flicker, jitter, low quality"
  };
